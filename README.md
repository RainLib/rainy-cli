...
RAINY_REPO=owner/repo RAINY_VERSION=v0.5.4 sh scripts/install.sh
...
```powershell
.\scripts\install.ps1 -Repo owner/repo -Version v0.5.4 -InstallDir "$HOME\.rainy\bin"
...
Run `rainy self update --apply` to update, or `rainy self skip 0.5.4 --apply` to skip this version.
```
...
```bash
git tag -a v0.5.4 -m "Rainy CLI v0.5.4"
